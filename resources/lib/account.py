...
class Account:
  ...
  def filter_games(self, games, filter_type=None):
      ...
      for game in games['results']:
        try:
          ...
          pre_game_home_start = game['prePostShows']['home']['preGame']['startTime'] if game['prePostShows']['home']['preGame']['hasShow'] else ''
          pre_game_away_start = game['prePostShows']['away']['preGame']['startTime'] if game['prePostShows']['away']['preGame']['hasShow'] else ''
          filtered_game = {
  ...
  def get_guide_xml(self, url_base):
      ...
      for game in games:
          game_start = self.utils.dateToString(self.utils.stringToDate(game['start'], "%Y-%m-%dT%H:%M:%S%z", True), "%Y%m%d%H%M%S %z")
          game_stop = self.utils.dateToString(self.utils.add_time(self.utils.stringToDate(game['start'], "%Y-%m-%dT%H:%M:%S%z"), hours=3), "%Y%m%d%H%M%S %z")
          game_original_air_date = self.utils.dateToString(self.utils.stringToDate(game['start'], "%Y-%m-%dT%H:%M:%S%z", True), "%Y-%m-%d %H:%M:%S")
          away_team_id = game['teamIds'][0]
          away_team_name = self.utils.get_cached_team_name(away_team_id)[0][0]
          home_team_id = game['teamIds'][1]
          home_team_name = self.utils.get_cached_team_name(home_team_id)[0][0]
          for teamId in game['teamIds']:
              ...
              if len(preGameStart) > 0:
                  ...
                  pregame_start = self.utils.dateToString(self.utils.stringToDate(preGameStart, "%Y-%m-%dT%H:%M:%S%z", True), "%Y%m%d%H%M%S %z")
                  pregame_original_air_date = self.utils.dateToString(self.utils.stringToDate(preGameStart, "%Y-%m-%dT%H:%M:%S%z", True), "%Y-%m-%d %H:%M:%S")
                  title = "Pre-Game Show"
                  xml_output += self.format_program(teamId, pregame_start, pregame_stop, title, subtitle, description, game['thumb'], pregame_original_air_date, game['gamePk'], away_team_name, home_team_name)

              title = 'MLB Baseball'
              xml_output += self.format_program(teamId, game_start, game_stop, title, subtitle, description, game['thumb'], game_original_air_date, game['gamePk'], away_team_name, home_team_name)

        
      xml_output += '''</tv>'''
      return xml_output.replace('&', '&amp;')

  def format_program(self, teamId, start, stop, title, subtitle, description, icon, original_air_date, gamePk, away_team_name, home_team_name):
      return '''
        ...
        </programme>'''.format(channel_id=self.get_channel_id(teamId), start=start, stop=stop, title=title, subtitle=subtitle, description=description, icon=icon, teamId=teamId, original_air_date=original_air_date, gamePk=gamePk, away_team_name=away_team_name, home_team_name=home_team_name)